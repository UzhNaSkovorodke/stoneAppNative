...
class Chat extends Component {
    ...
    checkContains = (fileUri) =>
        this.state.data.reduce((sum, current) => (current.fileUri === fileUri ? sum + 1 : sum), 0) >
        0
    ...
    renderImageInitials = ({ userName, userPatronymic }) => (
        <View style={[styles.profileImage]}>
            <Text style={styles.textProfileImage}>
                {`${this.getUserInitial(userName)}${this.getUserInitial(userPatronymic)}`}
            </Text>
        </View>
    )
        ...
        return (
            <View style={{ flex: 1 }}>
                ...
                <FlatList
                    ...
                    renderItem={({ item }) =>
                        item.key > -1 ? (
                            <ImageContainer onRemoveItem={this.removeData} idComponent={item} />
                        ) : (
                            <View style={styles.space} />
                        )
                    }
                ...
                {appealState === 'Закрыто' ? (
                    ...
                    <View style={styles.closedWrapper}>
                        <TouchableOpacity style={styles.clipButton} onPress={this.onClipPress}>
